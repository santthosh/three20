...
@synthesize launcherNavigationTopViewController = _launcherNavigationTopViewController;
...
-(id)init {
	if (self = [super init]) {
		...
		_overlayView = nil;
		_launcherNavigationTopViewController = nil;
	}
	return self;
}
...
-(void)addSubcontroller:(UIViewController*)controller animated:(BOOL)animated
	...
	if (!_launcherNavigationController) {
        _launcherNavigationController = [[UINavigationController alloc]
    ...
    UIBarButtonItem *launcherBarButtonItem=
        [[UIBarButtonItem alloc]
         initWithImage:TTIMAGE(@"bundle://Three20.bundle/images/launcher.png")
         style:UIBarButtonItemStyleBordered target:self action:@selector(dismissChild)];
        ...
        [_launcherNavigationController
         addSubcontroller:controller animated:animated transition:transition];
		self.launcherNavigationTopViewController = controller;
	}
}
...
-(void)navigationController:(UINavigationController *)navigationController
	...
	[_launcherNavigationTopViewController viewWillDisappear:animated];
...
-(void)navigationController:(UINavigationController *)navigationController
	...
	[_launcherNavigationTopViewController viewDidDisappear:animated];
	[viewController viewDidAppear:animated];
    self.launcherNavigationTopViewController = viewController;
}
...
-(void)layoutSubviews {
	...
	if (_headerView) {
		[_headerView setFrame:
         CGRectMake(0.0f, 0.0f, _headerView.frame.size.width,  _headerView.frame.size.height)];
		headerHeight = _headerView.frame.size.height;
	}

	if (_footerView) {
		footerHeight = _footerView.frame.size.height;
        [_footerView setFrame:CGRectMake(-10.0f, self.view.bounds.size.height - footerHeight,
                                         _footerView.frame.size.width,  footerHeight + 5)];
    }
    [self.view addSubview:_headerView];
	[self.view addSubview:_footerView];
	CGRect frame = [[UIScreen mainScreen] bounds];
	[_launcherView setFrame:
     CGRectMake(0.0f, headerHeight, frame.size.width,
                frame.size.height - 54 - headerHeight - footerHeight - 10)];
}
